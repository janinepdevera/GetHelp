...
base64.b64decode(cdb.password[9]).decode()

base64.b64decode(cdb.password[ind[0]]).decode() == password

def LogIn():
    cdb = pd.read_csv("../data/credentials_database.csv")
    ...
    while (len((cdb[cdb.username == username]['username'] == username).index) == 0):
       ...
       ind = index[cdb.username == username].tolist()
       password = str(input("\nEnter password: \n"))
       pw_encrypt = base64.b64encode(password.encode("utf-8")).decode()
#       while ~((cdb.password[ind] == password)[ind[0]]):
#       while ~(base64.b64decode(cdb.password[ind[0]]).decode() == password): 
       while ~((cdb.password[ind] == pw_encrypt)[ind[0]]):
           n2 += 1
           if (n2 < 3):
...
def NewUser():
    ...
    pw_encrypt = str(base64.b64encode(password.encode("utf-8")).decode())
#   pw_decrypt = base64.b64decode(pw_encrypt).decode()
